...
We have a wide range of beverages!`,
  ...
  useFrame((_state, delta) => {
    ...
    // Keyboard-controlled progress, emulate scroll 0..1
    const speedPerSecond = 0.08; // tune movement speed
    const activeDir =
      gestureDirection.current !== 0
        ? gestureDirection.current
        : inputDirection.current;
    progress.current += activeDir * speedPerSecond * delta;
    progress.current = Math.min(Math.max(progress.current, 0), 1);
    const scrollOffset = Math.max(0, progress.current);
  ...
  // Keyboard controls: W forward, S backward
  useEffect(() => {
    const onKeyDown = (e) => {
